...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if ((input->type == kTfLiteInt16 || input->type == kTfLiteInt8 || input->type == kTfLiteUInt8) &&
  ...
  if (input->type == kTfLiteFloat32) {
    ...
    switch (output->type) {
      case kTfLiteInt8:
        ...
        break;
      case kTfLiteUInt8:
        //printf("out kTfLiteUInt8 size %d, zero_point %d\n", size, output->params.zero_point);
        reference_ops::Requantize(
            GetTensorData<int8_t>(input), size, data->output_multiplier,
            data->output_shift, input->params.zero_point,
    ...
    switch (output->type) {
      case kTfLiteInt8:
        //printf("out kTfLiteInt8 size %d, zero_point %d\n", size, output->params.zero_point);
        reference_ops::Requantize(

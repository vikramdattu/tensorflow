...
// This method expects that output_data has been initialized.
template <>
inline bool ReduceSumImpl(const int8_t* input_data, const int* input_dims,
                          const int* output_dims, const int input_num_dims,
                          const int output_num_dims, const int* axis,
                          const int num_axis, int* input_iter,
                          int32_t* output_data) {
  auto reducer = [](const int32_t current, const int8_t in) -> int32_t {
    const int32_t actual_in = static_cast<int32_t>(in);
    return current + actual_in;
  };
  return Reduce<int8_t, int32_t>(input_data, input_dims, output_dims, input_num_dims,
                                 output_num_dims, axis, num_axis, input_iter, reducer,
                                 output_data);
}
...
// then divides it by the number of element in axis.
template <>
inline bool Mean(const int8_t* input_data, const int* input_dims,
  ...
  if (!ReduceSumImpl(input_data, input_dims, output_dims, input_num_dims,
                     output_num_dims, resolved_axis, num_resolved_axis,
                     temp_index, temp_sum)) {
    return false;
  }
  ...
  if (num_elements_in_axis > 0) {
    for (size_t idx = 0; idx < num_outputs; ++idx) {
      output_data[idx] =
          static_cast<int8_t>(temp_sum[idx] / static_cast<int32_t>(num_elements_in_axis));
    }
  }
  return true;
}
